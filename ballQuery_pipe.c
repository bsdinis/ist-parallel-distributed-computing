...
void allocate_tree() {
    ...
    for (long i = 0; i < n_nodes; i++) center[i] = &(_p_center[i * n_dims]);
}
...
int main(int argc, char *argv[]) {
    ...
    for (d = 0; d < n_dims; d++) point[d] = atof(argv[d + 1]);
    ...
    for (i = 0; i < n_nodes; i++) {
        ...
        for (d = 0; d < n_dims; d++) fscanf(fp, "%lf", &(center[i][d]));
    }
    ...
    for (d = 0; d < n_dims; d++) printf("%lf ", center[currBest][d]);
    printf("\n");
}
